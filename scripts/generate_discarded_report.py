...
OUTPUT_REPORT = "discarded_report.html"

def generate_report():
    print(f"Generando reporte HTML de descartados desde la base de datos: {DB_PATH}")
    
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # 1. Fetch all discarded sets (where level_1_entorno = 'Otros' and not an animal)
    cursor.execute("""
    SELECT 
    ...
    WHERE s.level_1_entorno = 'Otros' AND s.animal_level_1_habitat IS NULL
    ORDER BY s.confidence_score DESC, s.set_id ASC
    """)
    
    rows = cursor.fetchall()
    print(f"Encontrados {len(rows)} modelos descartados.")
    
    if not rows:
        print("No hay modelos descartados en la base de datos para reportar.")
        conn.close()
        return
...
<head>
    ...
    <title>Reporte de Diseños Descartados (No Vehículos)</title>
...
<body>
    <div class="container">
        <header>
            <h1>LEGO Discarded Designs Report (No-Vehicles)</h1>
            <div class="subtitle">Análisis de modelos categorizados como 'Otros' para validación de falsos negativos ({len(rows)} modelos cargados)</div>
        </header>
...
"""
    ...
    print(f"Reporte HTML de descartados guardado exitosamente en {OUTPUT_REPORT}!")
